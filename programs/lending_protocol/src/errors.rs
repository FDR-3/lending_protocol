...
//Error Codes
/* Having seperate error_code blocks was causing issues in the idl file error code generation where only one block of errors would be present.
This was causing this test: "Verifies that you can't Withdraw an Amount that Would Cause Your Debt to be More than 70% of the Value of your Collateral" to fail sometimes.
when it couldn't find the error since it is written differently and uses the idl file to find the error codes.
So putting them all under one error_code block.
#[error_code]
